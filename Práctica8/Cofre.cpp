...
Cofre::Cofre(int cuantosCaben)
        : _maxItems(cuantosCaben), _numItems(0) {
...
Cofre::Cofre(const Cofre &orig)
        : _maxItems(orig._maxItems), _numItems(orig._numItems) {
...
void Cofre::mete(Item *item) {
    if (item == nullptr) {
        throw std::invalid_argument("[Cofre::mete] se esta intentando aniadir al cofre un objeto null");
    }
...
Item &Cofre::consulta(int cual) {
    if (_numItems == 0) {
        throw EmptyContainer("[Cofre::consulta] se esta intentando consultar un cofre vacio");
    }
...
Item *Cofre::saca(int cual) {
    if (_numItems == 0) {
        throw EmptyContainer("[Cofre::consulta] se esta intentando sacar objetos de un cofre vacio");
    }
    if (cual <= 0 || cual > _numItems)
