...
void Mutante::addPoderPsiquico(string nombre, string descripcion,
                               string afectaA, float capacidadD, float lucidez) {
    addPoder(PoderPsiquico(nombre, descripcion, afectaA, capacidadD, lucidez));
}
...
bool Mutante::operator<(const Mutante &m1) {
    if (this->capacidadDestructivaTotal() < m1.capacidadDestructivaTotal()) {
        return 1;
    } else return 0;
}
