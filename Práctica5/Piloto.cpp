...
Piloto &Piloto::setNumMisiones(int numMisiones) {
    if (numMisiones <= 0) {
        throw std::invalid_argument("Piloto::setNumMisiones: el número de misiones no"
                                    " puede ser negativo");
    }
...
Piloto &Piloto::setFechaUltimaMision(long fechaUltimaMision) {
    if (this->getNumMisiones() == 0 && (_incidenciasUltimaMision != "")) {
        throw std::invalid_argument("Piloto::getFechaUltimaMision(): el piloto tiene 0 misiones realizadas");
    } else {
